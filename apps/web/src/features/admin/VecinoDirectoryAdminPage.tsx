...
export function VecinoDirectoryAdminPage() {
  ...
  const statsQuery = useQuery({
    queryKey: ["vecinoDirectory", "stats"],
    queryFn: async () => (await api.get<DirectoryStatsDto>("/vecino-directory/stats")).data
  });
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const { data } = await api.post<{ total: number; originalFilename: string }>(
        "/vecino-directory/upload",
        form,
    ...
    onSuccess: (data) => {
      setNotice(`Base reemplazada: ${data.total} vecinos cargados desde «${data.originalFilename}».`);
      setError(null);
      setPendingFile(null);
      setShowConfirm(false);
      if (fileRef.current) fileRef.current.value = "";
      void queryClient.invalidateQueries({ queryKey: ["vecinoDirectory", "stats"] });
    },
    onError: (err) => {
      setNotice(null);
    ...
    <section>
      <div className="page-header">
        <div className="page-header__copy">
          <h1 className="display-sm">Directorio de vecinos</h1>
          <p className="lead page-header__lead">
            Base global de vecinos. Se usa como reserva cuando una persona no está en la base del evento: en terminal
      ...
      <article className="card" style={{ marginTop: "1.25rem" }}>
        ...
        <p style={{ color: "var(--on-surface-variant)", marginTop: 0 }}>
          Subí un XLSX con columnas: NOMBRE, APELLIDO, DNI, Domicilio, Comuna, Teléfono, Correo, Q participaciones, Q
          Reclamos, Cod V. La carga <strong>reemplaza por completo</strong> el directorio anterior.
        </p>
        <input
          ref={fileRef}
          type="file"
      ...
      <ConfirmTypeDialog
        open={showConfirm}
        title="Reemplazar directorio de vecinos"
